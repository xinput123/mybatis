...
public class MapperBuilderAssistant extends BaseBuilder {
    ...
    public ResultMap addResultMap(
        ...
        if (extend != null) {  // 针对extend属性的处理
            ...
            // 如果当前<resultMap>标签中定义了<constructor>节点，则不需要使用父ResultMap中记录
    ...
    public ResultMapping buildResultMapping(
            ...
            boolean lazy) {
        // 解析type中的property属性的类型，也就是<resultMap>转换后的Java对象中property属性的类型
        Class<?> javaTypeClass = resolveResultJavaType(resultType, property, javaType);
        // 解析typeHandler属性指定的TypeHandler对象
        TypeHandler<?> typeHandlerInstance = resolveTypeHandler(javaTypeClass, typeHandler);
        // 解析column属性值，当column是"{prop1=col1,prop2=col2}"形式时，会解析成ResultMapping
        // 对象集合，column的这种形式主要用于嵌套查询的参数传递，嵌套查询后面会详细介绍
